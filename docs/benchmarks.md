...
Android는 `engine` flavor를 사용한다.

- `hermesRelease`: 기본 기준선이며 앱 ID는 `com.iris.bench`다.
- `irisRelease`: 앱 ID는 `com.iris.bench.iris`다. `IRIS_ENGINE_AAR`가 실제 RN 호환 Iris 엔진 artifact를 가리킬 때만 빌드한다.
