...
릴리스 성능 주장을 하려면 물리 기기에서 실행한 release 앱 로그의 `IRIS_BENCHMARK_ARTIFACT`를 `mise run bench-extract-hermes-release`로 추출해 `artifacts/bench/hermes-release-baseline.json`에 남긴다. 이 추출은 release build, Hermes, New Architecture, TurboModule number/string case를 모두 요구한다. RN 0.85 bridgeless Android에서는 `global.__turboModuleProxy`가 노출되지 않을 수 있으므로 전역 proxy 플래그가 아니라 실제 Codegen TurboModule benchmark case 실행 여부를 기준으로 삼는다. 산출물에는 다음 조건을 기록해야 한다.
