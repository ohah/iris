...
const defaultOutputPath = "artifacts/bench/strict-hbc-profile.txt";
const defaultJsonOutputPath = "artifacts/bench/strict-hbc-profile.json";
...
type ProfileCaseArtifact = {
  id: string;
  sourcePath: string;
  hbcPath: string;
  report: string;
  parsed: ParsedProfileReport;
};
...
function readField(report: string, startMarker: string, endMarker: string) {
  ...
  if (start < 0) {
    throw new Error(`profile report is missing ${startMarker}`);
  }
  const valueStart = start + startMarker.length;
  const end = report.indexOf(endMarker, valueStart);
  if (end < 0) {
    throw new Error(`profile report is missing ${endMarker}`);
  }
...
function readBracketList(report: string, name: string) {
  ...
  if (start < 0) {
    throw new Error(`profile report is missing ${name}`);
  }
  ...
  throw new Error(`profile report has an unterminated ${name} list`);
}
...
function parseProfileReport(report: string): ParsedProfileReport {
  return {
    ...
    jumpsTaken: Number(readField(report, ", jumpsTaken=", ", topOpcodes=[")),
...
run(cargo, ["build", "--release", "-p", "iris-hbc", "--bin", "hbc-profile"]);

const lines = [
  "Strict HBC scalar execution profile",
  "Generated by tools/bench/run-strict-hbc-profile.ts",
  "",
];
const cases: ProfileCaseArtifact[] = [];
...
for (const sourcePath of casePaths) {
  ...
  const report = capture(resolve(root, "target/release/hbc-profile"), [hbcPath]);
  ...
  cases.push({
    ...
    report,
    parsed: parseProfileReport(report),
  });
  console.log(`${id}: profiled`);
}

mkdirSync(dirname(outputPath), { recursive: true });
writeFileSync(outputPath, `${lines.join("\n")}\n`);
console.log(`Strict HBC profile artifact: ${outputPath}`);
...
writeFileSync(
  ...
  `${JSON.stringify(
    {
      schemaVersion: "iris.benchmark.strict-hbc-profile.v1",
      generatedBy: "tools/bench/run-strict-hbc-profile.ts",
...
console.log(`Strict HBC profile JSON artifact: ${jsonOutputPath}`);
